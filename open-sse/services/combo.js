 ...
 * turns. If a combo silently switches provider mid-session, the new provider
 * receives tool ids it never issued. Remember last successful model per combo
 * session and try it first on later tool turns.
 * @type {Map<string, { model: string, lastUsed: number }>}
 */
const comboStickyStore = new Map();

const STICKY_TTL_MS = 90 * 60 * 1000;
...
export function detectRequiredCapabilities(body) {
  ...
  // Search-like tools are soft capabilities. They should reorder when available,
  // but never drop fallback models.
  for (const tool of body.tools || []) {
    const type = tool?.type || tool?.function?.name || tool?.name;
    if (typeof type === "string" && /web_search|search/i.test(type)) required.add("search");
  }
...
export async function handleComboChat({ body, models, handleSingleModel, log, comboName, comboStrategy, comboStickyLimit = 1, autoSwitch = true }) {
  // Coding agents send tools and need stable priority order + provider continuity.
  // Preserve request body/tools and prefer last successful model in same session.
